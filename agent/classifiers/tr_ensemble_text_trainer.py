...
def train_loop(model, learning_rate, epsilon, batch_size, epochs, device, train_ds, val_ds, use_early_stopping=True):
    ...
    for epoch in range(epochs):
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            batch_ffnn_ids =            batch[2].to(device)
            batch_labels =              batch[3].to(device)

            batch_pred = model(batch_input_ids, batch_attention_masks, None, batch_ffnn_ids)
